...
void Led_Process(){
	if(Seg_Mode == 0){
		Led_Disp(0,0);
		if(Time_200ms == 0)  		Led_Disp(0,1);
		if(Total_Frequence>ChaoXian_Data){
			Led_Disp(1,0);
			if(Time_200ms == 0) Led_Disp(1,1);
		}
...
void Init_Time(){
	TMOD = 0x06;		//设置定时器模式
		TL1 = 64536%256;		//设置定时初值
		TH1 = 64536/256;		//设置定时初值
	TF1 = 0;	TF0 = 1;	//清除TF0标志
...
void Update_Frequence(){
	...
	Total_Frequence = 10*Count_Frequence + JiaoZhun_Data;
	if(Total_Frequence >= 0){
...
void Time1_Serve() interrupt 3{
    ...
    if(Seg_Slow_Down == 100)		Seg_Slow_Down = 0;
		if(++Time_200ms == 200)  Time_200ms = 0;
		if(++Time_100ms == 100){
			Time_100ms = 0;
			Update_Frequence();
		}
